...
void utils_binarioNaTela(char *nomeArquivoBinario) {
	 ...
	 * para leitura e depois fechar (você não vai perder pontos por
	 * isso se usar ela). */
	...
	if(nomeArquivoBinario == NULL || !(fs = fopen(nomeArquivoBinario, "rb"))) {
		fprintf(stderr, "ERRO AO ESCREVER O BINARIO NA TELA"
			" (função binarioNaTela): não foi possível abrir"
			" o arquivo que me passou para leitura. Ele existe"
			" e você tá passando o nome certo? Você lembrou de"
...
void utils_trim(char *str) {
	...
	*	Ela remove do início e do fim da string todo tipo de
	*	espaçamento (\r, \n, \t, espaço, ...). Por exemplo:
	*
...
void utils_scan_quote_string(char *str) {
	...
	*	entre aspas ("). Chame ela na hora que for ler tal
	*	campo. Por exemplo:
	*
	*	A entrada está da seguinte forma:
	*		nomeDoCampo "MARIA DA SILVA"
	*
	*	Para ler isso para as strings já alocadas str1 e str2 do seu
	*	programa, você faz:
